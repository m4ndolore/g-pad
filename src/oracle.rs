//! The oracle behind the pad — the thing that reads handwriting and
...
//! `RIDDLE_OPENAI_MODEL`) to use HTTP; otherwise g-pad falls back to pi.
...
const DATA_DIR: &str = "/home/root/g-pad-data";
const NODE_BIN: &str = "/home/root/node/bin";

const PERSONA: &str = "You are the voice of this writing pad. Someone writes to you with a pen; their words appear as ink on the page. Reply on the same page: clear, concise, useful. Keep replies SHORT: one to three sentences, like ink appearing on paper. Never mention images, photos, models or AI; you only ever perceive words written on the page. If the writing is illegible, say the ink blurred. Always answer in the language the writer used.";

/// The pad's voice: RIDDLE_PERSONA_FILE, else persona.txt next to the
/// binary (the launch scripts cd into the bundle dir), else the default.
pub fn persona() -> String {
    let path = std::env::var("RIDDLE_PERSONA_FILE").unwrap_or_else(|_| "persona.txt".into());
    match std::fs::read_to_string(&path) {
        Ok(s) if !s.trim().is_empty() => {
            eprintln!("g-pad: persona loaded from {path}");
            s.trim().to_string()
        }
        _ => PERSONA.to_string(),
    }
...
/// What the oracle streams back to the pad.
#[derive(Debug, PartialEq)]
pub enum Event {
    /// A sentence (or more) of the pad's reply — ink it.
    Ink(String),
        ...
        if !self.route_checked {
            ...
            if lead.starts_with(SHOW_OPEN) {
                ...
                match n.and_then(|n| self.catalog_ids.get(n.wrapping_sub(1)).copied()) {
                    ...
                    None => out.push(Err(format!("the pad lost that page ({inner})"))),
                }
...
/// The pad's oracle. A backend-agnostic front over the two oracle kinds.
pub enum Oracle {
    Http(HttpOracle),
    Pi(PiOracle),
}
...
impl Oracle {
    ...
    pub fn spawn(remember: bool) -> std::io::Result<Self> {
        if std::env::var("RIDDLE_OPENAI_KEY").is_ok() {
            eprintln!("g-pad: oracle = OpenAI-compatible HTTP");
            Ok(Oracle::Http(HttpOracle::new(remember)?))
        } else {
            eprintln!("g-pad: oracle = pi (set RIDDLE_OPENAI_KEY for the HTTP backend)");
...
impl PiOracle {
    ...
    pub fn spawn(remember: bool) -> std::io::Result<Self> {
        ...
        let pid = child.id();
        eprintln!("g-pad: oracle pi rpc spawned (pid {pid}, bin {pi_bin})");
        ...
        thread::spawn(move || {
            ...
            let emit = |events: Vec<Result<Event, String>>| {
                ...
                if let Some(t0) = asked_r.lock().unwrap().take() {
                    eprintln!("g-pad: oracle first chunk +{}ms", t0.elapsed().as_millis());
                }
    ...
    pub fn ask_with_model(&self, png_path: &str, ctx: &TurnContext, tx: Sender<Result<Event, String>>, model_override: Option<&str>) {
        ...
        thread::spawn(move || {
            ...
            let resp = match request("max_tokens") {
                Err(ureq::Error::Status(400, r)) => {
                    ...
                    if detail.contains("max_completion_tokens") {
                        eprintln!("g-pad: endpoint wants max_completion_tokens; retrying");
            ...
            let mut emit = |events: Vec<Result<Event, String>>| {
                for ev in events {
                    if first {
                        eprintln!("g-pad: oracle first chunk +{}ms", asked.elapsed().as_millis());
                        first = false;
                    }
                    let _ = tx.send(ev);
                }
