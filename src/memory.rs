//! The pad's memory. Every finished turn is kept — the writer's actual pen
//! strokes, a transcription of their words, and the pad's reply — so a later
//! request ("show me what I wrote about the garden") can restore the page
//! back in the writer's own hand.
//!
//! Everything lives on the tablet, in plain files under
//! `/home/root/g-pad-data/memories` (override: `RIDDLE_MEMORY_DIR`):
//!
...
//!
//! Delete the directory and the pad forgets. `RIDDLE_MEMORY=off` disables
//! remembering entirely (no storage, no context sent with requests).

use std::io::Write as _;
use std::path::PathBuf;

/// Newest memories the pad keeps. Older pages are forgotten (pruned).
...
/// A sitting at the pad: consecutive turns closer than six hours.
...
impl MemoryStore {
    /// Open (or start) the pad's memory. Returns None when memory is off.
    pub fn open() -> Option<Self> {
        ...
        let dir = std::env::var("RIDDLE_MEMORY_DIR")
            .map(PathBuf::from)
            .unwrap_or_else(|_| PathBuf::from("/home/root/g-pad-data/memories"));
        if let Err(e) = std::fs::create_dir_all(&dir) {
            eprintln!("g-pad: memory disabled ({}: {e})", dir.display());
            return None;
        }
    ...
    pub fn append(&mut self, id: u64, transcript: &str, reply: &str, strokes: &Strokes) {
        ...
        if let Err(e) = std::fs::write(self.strokes_path(id), lines) {
            eprintln!("g-pad: memory strokes not kept: {e}");
        }
        ...
        if let Err(e) = appended {
            eprintln!("g-pad: memory not kept: {e}");
            return;
        }
    ...
    fn prune(&mut self) {
        ...
        if let Err(e) = std::fs::write(self.index_path(), out) {
            eprintln!("g-pad: memory prune failed: {e}");
        }
    }
